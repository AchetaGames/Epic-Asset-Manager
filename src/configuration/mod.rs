...
impl Configuration {
    pub(crate) fn new() -> Configuration {
        ...
        match conf.egs.merge(config::File::new(&conf.path.clone().unwrap(), config::FileFormat::Json)) {
            Ok(_) => {}
            Err(e) => {
        ...
        match conf.env.merge(config::Environment::with_prefix("EAM")) {
            Ok(_) => {}
            Err(e) => { println!("Failed to load properties from Environment: {}", e) }
        }
        ...
        if let Some(path) = self.path.clone() {
            ...
            match File::create(config_path.join("user.json")) {
                Ok(mut user_file) => {
                    ...
                    user_file.write(serde_json::to_string(&self.user_data).unwrap().as_bytes().as_ref());
                }
                Err(_) => {}
            }
        }
