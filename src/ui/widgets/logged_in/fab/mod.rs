...
pub mod imp {
    ...
    #[template(resource = "/io/github/achetagames/epic_asset_manager/fab/library.ui")]
    pub struct FabLibraryBox {
        #[template_child]
        pub details: TemplateChild<asset_detail::FabAssetDetails>,
        #[template_child]
        pub sidebar: TemplateChild<sidebar::FabSidebar>,
        #[template_child]
...
impl FabLibraryBox {
    ...
    fn populate_model(list_item: &gtk4::ListItem) {
        ...
        let child = list_item.child().unwrap().downcast::<FabAsset>().unwrap();
        child.set_data(&data);
    }
...
impl Refresh for FabLibraryBox {
    fn run_refresh(&self) {
        self.fetch_assets();
    }
